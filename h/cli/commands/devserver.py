...
def devserver(https, web, ws, worker, assets, beat):
    ...
    m = Manager()
    if web:
        m.add_process('web', 'newrelic-admin run-program gunicorn --name web --reload --paste conf/development-app.ini %s' % gunicorn_args)

    if ws:
        m.add_process('ws', 'newrelic-admin run-program gunicorn --name websocket --reload --paste conf/development-websocket.ini %s' % gunicorn_args)
